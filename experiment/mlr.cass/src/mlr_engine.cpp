...
float ParseLibSVMLine(const char* line, 
  ...
  char *ptr = 0, *endptr = 0;
  // Read label.
  float label = strtof(line, &endptr);
  label = label_one_based ? label - 1 : label;
  size_t size = strlen(line);
  ptr = endptr;

  while (isspace(*ptr) && ptr - line < size) ++ptr;
  while (ptr - line < size) {
...
void LogErrorAndDie(CassFuture *future){
  const char* message;
  size_t message_length;
  cass_future_error_message(future, &message, &message_length);
  LOG(FATAL) << "[client " << FLAGS_client_id << "]: "<< "CassError: " << message;
}


void MLREngine::ReadDataFromCassandra() {
  petuum::HighResolutionTimer read_timer, tmp_timer;
  float load_time = 0.0f, connect_time = 0.0f, parse_time = 0.0f;
  ...
  // in case receiving a 'Request timed out' error when executing query, set it larger.
  cass_cluster_set_request_timeout(cluster, 600*1000);

  future = cass_session_connect(session, cluster);
  if (cass_future_error_code(future) != CASS_OK) {    
    cass_cluster_free(cluster);
    cass_session_free(session);
    LOG(FATAL) << "connect_session failed!";
    return;
  }
  cass_future_free(future);
  connect_time = read_timer.elapsed();
  LOG(INFO) << "[client " << FLAGS_client_id << "]: " << "connection established in " << connect_time << " seconds" << std::endl;
  
  read_timer.restart();
  char query[1024];
  const char* dbname = strrchr(FLAGS_train_file.c_str(), '/') + 1;
  if(FLAGS_global_data)
    std::sprintf(query, "SELECT content FROM %s;", dbname);
  else
    std::sprintf(query, "SELECT content FROM %s WHERE partition_key = %d;", dbname, FLAGS_client_id);
  LOG(INFO) << "[client " << FLAGS_client_id << "]: " << query;
  CassStatement* statement = cass_statement_new(query, 0);
  // important for retrieving dataset with TOO MANY ROWS(over a million),
  // or there will be 'Request timed out' or 'Operation timed out'!
  cass_statement_set_paging_size(statement, 500000);

  // retreiving data
  bool has_more_pages = false;
  int32_t i = 0;
  std::vector<int32_t> feature_ids(feature_dim_);
  std::vector<float> feature_vals(feature_dim_);
  load_time = 0.0f;
  do{
    tmp_timer.restart();
    future = cass_session_execute(session, statement);
    if (cass_future_error_code(future) != CASS_OK) {
      LogErrorAndDie(future);
      break;
    }
    load_time += tmp_timer.elapsed();
    const CassResult *result = cass_future_get_result(future);
    CassIterator* iterator = cass_iterator_from_result(result);
    cass_future_free(future);
    for (;cass_iterator_next(iterator); i++) {
      const CassRow* row = cass_iterator_get_row(iterator);
      const CassValue* value = cass_row_get_column_by_name(row, "content");
      const char* content;
      size_t content_length;
      cass_value_get_string(value, &content, &content_length);
      tmp_timer.restart();
      float label = ParseLibSVMLine(content, feature_ids,
          feature_vals, feature_one_based_, label_one_based_);
      
      train_labels_[i] = label;
      if (train_labels_[i] == -1)
        train_labels_[i] = 0;
      train_features_[i] = new petuum::ml::SparseFeature<float>(feature_ids, feature_vals,
          feature_dim_);
      parse_time += tmp_timer.elapsed();
    }
  ...
  }while(has_more_pages); // end of while
  ...
  LOG(INFO) << "[client " << FLAGS_client_id << "]: "<< "Read " << i << " instances in "
    << read_timer.elapsed() << " seconds.";
  LOG(INFO) << "[client " << FLAGS_client_id << "]: "<< "connect time: " << connect_time<< " seconds";
  LOG(INFO) << "[client " << FLAGS_client_id << "]: "<< "Load    time: " << load_time << " seconds";
  LOG(INFO) << "[client " << FLAGS_client_id << "]: "<< "parse   time: " << parse_time << " seconds";
}
// add end
