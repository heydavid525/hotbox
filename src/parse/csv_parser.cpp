...
// Very important !!! No invalid input is allowed here
// If read a blank token, put the value 0.0 in the field
...
void CSVParser::SetConfig(const ParserConfig& config) {
  ...
  if (config.has_csv_config()) {
    ...
    // Oct 31 add two config parameter herer
    // header_position_ is the location of the header line: 0 no header
    // 1 header at the first line etc
    header_position_ = csv_config.header_position();
    first_column_label_ = csv_config.first_column_label();
  }
    LOG(INFO) << "header_position_: " << header_position_;
    LOG(INFO) << "first_column_label_: " << first_column_label_;
