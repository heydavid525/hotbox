...
std::function<void(std::vector<TransDatum*>*)>
TfTransform::GenerateBatchTransform(const TransformParam& param) const {
  const TfTransformConfig& config =
    param.GetConfig().tf_transform();
  const std::multimap<std::string, WideFamilySelector>& 
    w_family_selectors = param.GetWideFamilySelectors();
  ...
  session_config.input_dim = input_dim;
  TfSession session(session_config);
  return [session, selector, input_dim] (std::vector<TransDatum*>* batch) mutable {
    std::vector<std::vector<float>> v(batch->size());
    //const std::vector<float>& out = session.Transform(v);
    for (int i = 0; i < batch->size(); ++i) {
      v[i] = std::move(GetDenseVals(*((*batch)[i]), selector));
    }
    const std::vector<std::vector<float>>& out = session.Transform(v);
    for (int i = 0; i < batch->size(); ++i) {
      (*batch)[i]->SetFeatureValRelativeOffset(0, out[i]);
    }
  };
}
